...
def contourPlot(ax, f, size_x_1,size_x_2, size_y_1,size_y_2):
    # Подготовка к рисованию, настраиваем оси x и y
    x1 = np.arange(size_x_1, size_x_2 + 0.1, 0.1)
    m = len(x1)
    y1 = np.arange(size_y_1, size_y_2 + 0.1, 0.1)
    n = len(y1)
...
def draw(coords, nsteps, f, flag, size_x_1=-4,size_x_2=4, size_y_1=-4,size_y_2=4):
    fig, ax = plt.subplots()
    fig.suptitle('BFGS method each step visualisation & Countour plot')
    plt.xlim(size_x_1, size_x_2)
    plt.ylim(size_y_1, size_y_2)
    plt.gca().set_aspect('equal', adjustable='box')
    dfpDraw(ax, coords, nsteps)
    contourPlot(ax, f, size_x_1,size_x_2, size_y_1, size_y_2)
    name = "plot" + flag + ".png"
    fig.savefig(name)
